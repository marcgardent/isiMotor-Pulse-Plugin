...
#define PLUGIN_NAME "isiMotor_Pulse.dll"
...
static inline void PluginLog(const char* fmt, ...) {
    ...
    if (g_hModule) {
        ...
        if (lastSlash) {
            std::strcpy(lastSlash + 1, "isiMotor_Pulse.log");
        }
    }
#endif
    FILE* f = path[0] ? std::fopen(path, "a") : nullptr;
    if (!f) f = std::fopen("Plugins/isiMotor_Pulse.log", "a");
    if (!f) f = std::fopen("isiMotor_Pulse.log", "a");
    if (!f) f = std::fopen("UserData/Log/isiMotor_Pulse.log", "a");
    if (!f) f = std::fopen("UserData/isiMotor_Pulse.log", "a");
    if (!f) return;
    va_list args;
    va_start(args, fmt);
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
        g_hModule = hModule;
        PluginLog("=== isiMotor_Pulse.dll Loaded (DLL_PROCESS_ATTACH, PID=%lu) ===", static_cast<unsigned long>(GetCurrentProcessId()));
    }
    return TRUE;
}
    ...
    void Startup(long version) override {
        ...
        for (unsigned char packetType : kOutboundPacketTypes) {
            try {
                ...
                PluginLog("[Pulse] PUB socket for packet type %d bound on %s", packetType, endpoint);
            } catch (const zmq::error_t& e) {
                PluginLog("[Pulse] Failed to bind PUB socket for packet type %d: %s", packetType, e.what());
        ...
        if (config.enableInboundControl) {
            try {
                ...
                inboundBound = true;
                PluginLog("[Pulse] Inbound commands SUB bound on %s", inboundEndpoint);
            } catch (const zmq::error_t& e) {
                PluginLog("[Pulse] Failed to bind inbound SUB socket: %s", e.what());
                inboundBound = false;
            }
        }
        ...
        if (resolvedConfigPath[0] != '\0') {
            if (configWatcher.Start(resolvedConfigPath)) {
                PluginLog("[Pulse] Config file watcher started on: %s", resolvedConfigPath);
            }
        }
#endif
    }
    ...
    void UpdateScoring(const ScoringInfoV01 &info) override {
        ...
        if (configWatcher.CheckAndClearDirty()) {
            ...
            if (std::strcmp(prevHost, config.tcpHost) != 0 || prevBasePort != config.tcpBasePort) {
                for (unsigned char packetType : kOutboundPacketTypes) {
                    ...
                    try {
                        ...
                        PluginLog("[Pulse] Hot-reload: packet type %d endpoint updated to %s", packetType, newEndpoint);
                    } catch (const zmq::error_t& e) {
                        PluginLog("[Pulse] Hot-reload: failed to rebind packet type %d endpoint to %s: %s", packetType, newEndpoint, e.what());
                    }
                }
            }
            ...
            if (prevInboundPort != config.inboundTcpPort) {
                PluginLog("[Pulse] Hot-reload: InboundTcpPort changed (%d -> %d), restart required to apply",
                          prevInboundPort, config.inboundTcpPort);
            }

            PluginLog("[Pulse] Configuration hot-reloaded from: %s", resolvedConfigPath);
        }
#endif
...
struct MsvcPluginInstance {
    ...
    IsiMotorPulsePlugin impl;         // Offset 16: Concrete Plugin Implementation
};
#pragma pack(pop)

extern const MsvcInternalsVTable g_MsvcVTable;

static inline IsiMotorPulsePlugin* GetPlugin(void* self) {
