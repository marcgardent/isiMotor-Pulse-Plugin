...
class TestBumpVersion(unittest.TestCase):
    ...
    def test_bump_all_files_mock_workspace(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn(tmp_root / "binding" / "python" / "isimotor-pulse-client" / "pyproject.toml", updated)
            self.assertIn(client_dir / "__init__.py", updated)
            self.assertIn(tmp_root / "binding" / "python" / "isimotor-pulse-manager" / "pyproject.toml", updated)
            ...
            self.assertIn(
                'version = "2.0.0"',
                (tmp_root / "binding" / "python" / "isimotor-pulse-client" / "pyproject.toml").read_text(
                    encoding="utf-8"
                ),
            )
