...
def decode_pit_menu(data: bytes, offset: int = 0) -> PitMenu | None:
    """Decodes a 76-byte PitMenu packet (Type 6)."""
    if len(data) - offset < PIT_MENU_SIZE:
        return None

    cat_idx, raw_cat_name, choice_idx, raw_choice_str, num_choices = struct.unpack_from(PIT_MENU_STRUCT, data, offset)
    return PitMenu(
