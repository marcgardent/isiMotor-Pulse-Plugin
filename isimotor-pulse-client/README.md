...
# isimotor-pulse-client

Official Python client and high-performance binary decoder for **isiMotor-Pulse-Plugin** (compatible with **Le Mans Ultimate** and **rFactor 2**).
...
```python
from isimotor_pulse_client import DomainSinglePacketClientFactory
...
```python
from isimotor_pulse_client import RawSinglePacketClientFactory
...
```python
from isimotor_pulse_client import DtoSinglePacketClientFactory
...
A CLI is also available, installed as `isi-install` / `isi-installer` (or `python -m isimotor_pulse_client.install.cli`):
