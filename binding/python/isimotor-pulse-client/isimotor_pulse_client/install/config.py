...
def configure_game_json(game_dir: Path, plugin_dll_name: str = "isiMotor_Pulse.dll") -> bool:
    """
...
def read_plugin_json_variables(json_path: Path, plugin_name: str = "isiMotor_Pulse.dll") -> dict[str, Any]:
    """Reads and parses CustomPluginVariables.JSON for isiMotor_Pulse variables."""
...
def write_plugin_json_variables(
    ...
    plugin_dll_name: str = "isiMotor_Pulse.dll",
) -> tuple[bool, str]:
    """Writes or updates isiMotor_Pulse configuration variables inside CustomPluginVariables.JSON."""
    dll_key = plugin_dll_name if plugin_dll_name.lower().endswith(".dll") else f"{plugin_dll_name}.dll"
    base_name = dll_key.replace(".dll", "")
    alt_name = base_name.replace("_", "-")
...
def get_configuration_overview(
    ...
    dll_info: dict[str, Any] = {
        ...
        "filename": src_dll.name if src_dll else "isiMotor_Pulse.dll",
    ...
    for game_key, game_name, gdir in targets:
        plugin_dll = gdir / "Plugins" / "isiMotor_Pulse.dll"
        root_dll = gdir / "isiMotor_Pulse.dll"
