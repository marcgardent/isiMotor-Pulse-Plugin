# isiMotor Pulse Manager & Telemetry Diagnostics (Textual TUI)
...
> ℹ️ Steam detection, DLL install/uninstall and JSON configuration are implemented in the **[`isimotor-pulse-client`](../isimotor-pulse-client#-installer-api-isimotor_pulse_clientinstall)** package (`isimotor_pulse_client.install`) — the Manager UI is just one consumer of that public API. Use the client package directly if you want to automate installation without the TUI.
...
* **One-Click Actions & Clipboard Export:**
  * `📦 Copy DLL` (`k` key) : Automatically copies `isiMotor_Pulse.dll` to detected game folders and configures default JSON profiles.
...
isi-manager
# or:
python -m isimotor_pulse_manager

# Run game detector & plugin installer (provided by isimotor-pulse-client):
isi-install --status
# or:
python -m isimotor_pulse_client.install.cli
```
