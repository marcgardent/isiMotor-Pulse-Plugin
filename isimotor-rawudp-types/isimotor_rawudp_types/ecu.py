...
class EcuState:
    ...
    """True if Traction Control is actively cutting/intervening."""
    abs_active: bool = False
    """True if Anti-lock Braking System is actively modulating pressure."""

    tc_level: int = -1
    """TC level (-1 if unavailable, 0=off or 1..tc_max)."""
    tc_max: int = 0
    """Maximum TC setting available."""
    tc_cut: int = -1
    """TC engine power cut level (-1 if unavailable)."""
    tc_cut_max: int = 0
    """Maximum TC cut setting."""
    tc_slip: int = -1
    """TC slip angle allowance level (-1 if unavailable)."""
    tc_slip_max: int = 0
    """Maximum TC slip setting."""

    abs_level: int = -1
    """ABS level (-1 if unavailable, 0=off or 1..abs_max)."""
    abs_max: int = 0
    """Maximum ABS setting available."""

    motor_map: int = -1
    """Engine / Motor power map level (-1 if unavailable)."""
    motor_map_max: int = 0
    """Maximum motor map setting."""

    brake_migration: int = -1
    """Dynamic brake migration level (-1 if unavailable)."""
    brake_migration_max: int = 0
    """Maximum brake migration setting."""

    front_arb: int = -1
    """Onboard adjustable front anti-roll bar level (-1 if unavailable)."""
    front_arb_max: int = 0
    """Maximum front ARB setting."""
    rear_arb: int = -1
    """Onboard adjustable rear anti-roll bar level (-1 if unavailable)."""
    rear_arb_max: int = 0
